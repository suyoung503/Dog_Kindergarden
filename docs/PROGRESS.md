...
- **저장소:** 모노레포 단일 repo, `main` 브랜치. origin 최신 커밋 `b35e4c8`(push 완료) — 받은 예약 요청 내 가게 스코프 + 최근 본 가게 계정별 영속 + 문서 갱신

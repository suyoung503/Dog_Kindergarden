...
| 사장님↔손님 양방향 채팅 | ✅ 구현 완료 (백엔드 배포·curl 검증 완료, 시뮬레이터 확인 대기) |
| 역할별 계정 분리 + 마이페이지 내 가게 등록 | ✅ 구현 완료 (백엔드 배포·curl 검증 완료, 시뮬레이터 확인 대기) |
| README | ✅ 작성 완료 |
...
- **저장소:** 모노레포 단일 repo, `main` 브랜치. origin 최신 커밋 `2441930`(push 완료). 미커밋: 사장님↔손님 양방향 채팅 + 역할별 계정 분리·내 가게 등록 (커밋 예정)

...
### ✅ Task 5: 네이버 블로그 API 연동 (완료 — 2026-07-17)

당초 계획했던 developers.naver.com 검색 API는 **신규 제휴 중단** 상태라, **NCP NAVER API HUB**(네이버클라우드)에서 키를 발급해 연동했다 — 엔드포인트 `naverapihub.apigw.ntruss.com/search/v1/*`, 헤더 `X-NCP-APIGW-API-KEY-ID`/`-KEY`, 응답 포맷은 구 API와 동일. 키는 `Create.xcconfig`의 `NAVER_CLIENT_ID`/`NAVER_CLIENT_SECRET`. 블로그 후기(`NaverBlogService`)에 더해 같은 키로 **지역 검색 주소 보강**(`NaverLocalService`)까지 확장 — 상세는 `PORTFOLIO.md` §12.
...
| 기능 | 비고 |
|---|---|
| ~~`reviews` 테이블 삭제~~ | ✅ 완료 (2026-07-17) — 마이그레이션 `0012_drop_reviews.sql`, 배포 DB 적용 |
| ~~리뷰 요청 시스템 메시지~~ | ✅ 완료 (2026-07-12) — Cron Triggers 매일 KST 18시, 이용일 다음날 채팅방에 자동 메시지 |
| ~~사장님 취소 알림~~ | ✅ 완료 (2026-07-12) — 사장님 취소 시 고객 채팅방에 자동 메시지(`by_owner` 플래그) + 예약 신청 화면에 취소 가능 경고 문구 |
| ~~안 읽은 채팅 빨간 점~~ | ✅ 완료 (2026-07-12) — `chat_room_reads` 읽음 추적 + 홈 종 빨간 점 + 탭 시 채팅 목록 이동 |
| ~~리뷰 태그 지도 필터링~~ | ✅ 완료 (2026-07-17) — 홈 지도 태그 칩 다중 선택(AND) + 필터 UI |
| ~~사용자별 데이터 분리 검증~~ | ✅ 완료 (2026-07-12) — UserProfile 계정 간 잔존 수정 + iOS/서버 `?? 1` 폴백 전수 제거. API 무인증(user_id 신뢰)은 데모 한계로 FEATURES §13 기록 |
| ~~채팅/시스템 메시지 분리~~ | ✅ 완료 (2026-07-18) — 자동 메시지(sender 0)를 중앙 정렬 안내문으로 분리 |
...
| 기능 | 난이도 | 비고 |
|---|---|---|
| ~~알림장(diary)~~ | 중간 | ✅ 완료 (2026-07-17) — 예약당 타임라인, 사장님 작성/보호자 열람. **사진 첨부만 R2 활성화 후 추가**(스키마·API 계약 준비됨). ※ 예약 승인은 사장님 모드로 구현 완료 — 별도 업체 앱 계획은 폐기 |
...
Apple Developer Program 유료 멤버십($99/년) 등 고정비가 있어 수익 모델 고민이 필요하다는 문제의식에서 정리. 결론: 수익 모델 설계보다 사장님 쪽 실사용자 확보가 먼저 — 이 앱은 사장님이 설치해야 채팅·알림장·캘린더가 살아나는 구조라, 몇 곳이라도 실제로 써보게 하는 게 우선이고 수익화는 그 다음 단계로 미룬다.

- **견주 대상**: 예약 결제가 네이버예약 리다이렉트로 빠지는 구조(위 표의 "결제 시스템" 방향)라 앱이 결제에서 손을 떼므로 수수료를 뗄 방법이 없음.
- **사장님 대상 (더 현실적)**: 큐레이션 상단 노출/광고 슬롯, 또는 채팅·알림장·캘린더 등 사장님 도구를 월 구독제로(예: 채팅은 무료, 알림장+캘린더+통계는 유료). 둘 다 사장님 실사용자가 있어야 팔 수 있는 상품이라 실제 도입은 소수 가게 확보 이후로 미룬다.
